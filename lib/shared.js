...
shared.IATI_NS = 'http://purl.org/collections/iati/';
...
shared.getInferredTypes = function(uri, callback) {
  ...
  conn.getColValues(
    { query: query, reasoning: 'EL' }, function(err, resolvedTypes) {
      if (err) {
        return callback(err);
      }
...
shared.getDescriptionPath = function(resourceURI) {
  ...
  resourceURI = shared.expandPrefixes(resourceURI);
...
shared.getPreferredDatasetLabel = function(resource) {
...
shared.generateConditionsString = function(conditions, param) {
  ...
  _.each(conditions, function(value, rawKey) {
    ...
    if (rawKey === '@type') {
      ...
      throw new
        Error('Invalid value for conditions string: ' + value);
    }
...
shared.expandPrefixes = function(node) {
  var found = node;
